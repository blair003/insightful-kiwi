...
load_core_data <- function(config, force_rebuild = FALSE, refresh_weather = FALSE) {
  ...
  if (isTRUE(force_rebuild) && file.exists(cache_file)) {
    logger::log_info(
      "global_functions.R, deleting cached core_data for data package id %s from %s",
      package_id,
      cache_file
    )
    unlink(cache_file)
  ...
  if (file.exists(cache_file)) {
    logger::log_info(
      "global_functions.R, cache hit for data package id %s, loading core_data from %s",
      package_id,
    ...
    logger::log_info(
      "global_functions.R, cache miss for data package id %s, processing data...",
      package_id
    )
