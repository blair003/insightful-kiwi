        ...
        tags$div(
          ...
          if (isTRUE(include_marker_options) && isTRUE(trap_data_available)) {
            tagList(
              ...
              conditionalPanel(
                condition = "input.show_trap_check_frequency",
                ns = ns,
                tags$div(
    ...
    return(
      tagList(
        ...
        tags$div(
          ...
          if (isTRUE(include_prediction_option)) {
            conditionalPanel(
              condition = "input.timeline_view_mode === 'cumulative' && input.show_predicted_rai_surface",
              ns = ns,
              tags$div(
                class = "prediction-surface-options",
                radioButtons(
...
update_map <- function(map_id = NULL,
  ...
  if (nrow(obs_summary_location) > 0) {
    ...
    logger::log_debug(sprintf("mapping_module_server, %s update_map() has no results available", map_id))
    ...
    if ((!show_zero || nrow(active_locations) == 0) &&
      ...
      logger::log_debug(sprintf("mapping_module_server, %s update_map() creating popup saying no results", map_id))
      proxy %>% addControl(
