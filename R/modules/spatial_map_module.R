# R/modules/spatial_map_module.R
...
activity_pattern_bins <- function(step_size) {
  switch(step_size,
    "hour" = sprintf("%02d", 0:23),
    "diel" = c("Matutinal", "Diurnal", "Vespertine", "Nocturnal"),
    "day_night" = c("Day", "Night"),
    character(0)
  )
}

activity_pattern_bin_labels <- function(step_size) {
  if (identical(step_size, "hour")) sprintf("%02d:00", 0:23) else activity_pattern_bins(step_size)
}

activity_pattern_bin_column <- function(step_size) {
  switch(step_size,
    "hour" = "activity_pattern_hour_bin",
    "diel" = "diel_class",
    "day_night" = "day_night_class",
    NA_character_
  )
}

activity_pattern_window_label <- function(step_size, view_mode, current_index) {
  labels <- activity_pattern_bin_labels(step_size)
  current_label <- labels[[current_index]]
  # Hourly: show the hour as a range (e.g. "02:00 - 03:00"); the slider ticks keep
  # the short single-hour labels.
  if (identical(step_size, "hour")) {
    idx <- as.integer(current_index)
    start_hr <- (idx - 1L) %% 24L
    end_hr <- idx %% 24L
    if (identical(view_mode, "single") || identical(current_index, 1L)) {
      return(sprintf("%02d:00 - %02d:00", start_hr, end_hr))
    }
    return(sprintf("00:00 - %02d:00", end_hr))
  }
  if (identical(view_mode, "single") || identical(current_index, 1L)) {
    current_label
  } else {
    sprintf("%s to %s", labels[[1]], current_label)
  }
}

timeline_ui <- function(ns, points, value, step_size = "day", view_mode = "cumulative", activity_pattern_mode = FALSE) {
  point_count <- length(points)
  if (isTRUE(activity_pattern_mode) && step_size %in% c("hour", "diel", "day_night")) {
    selected_index <- 1L
    slider_labels <- activity_pattern_bin_labels(step_size)
  } else {
  ...
  if (point_count <= 1) {
    return(div(
      ...
      div(class = "timeline-progression-label", slider_label),
      transport,
      div(
...
timeline_time_points <- function(start_time, end_time, step_size, view_mode = "cumulative",
                                 weather_df = NULL, period_groups = NULL, activity_pattern_mode = FALSE) {
  ...
  if (isTRUE(activity_pattern_mode) && step_size %in% c("hour", "diel", "day_night")) {
    n <- length(activity_pattern_bins(step_size))
    return(as.POSIXct("1970-01-01", tz = timeline_actual_timezone()) + seq_len(n) * 3600)
  }
...
render_timeline_period_control <- function(period_status, monitoring_window = NULL,
  ...
  # monitoring_only: the season label/dates are redundant when the selection is a
  # single season, so show just the monitoring sub-window (or nothing if absent).
  if (isTRUE(monitoring_only)) {
    if (!nzchar(monitoring_line)) return(NULL)
    return(sprintf("<div class='map-season-badge'>%s</div>", monitoring_line))
  }
...
timeline_window_readout <- function(start_time, current_time, step_size, weather_df, view_mode = "single",
                                     activity_pattern_mode = FALSE, activity_pattern_index = NULL,
                                     window_label = "Timeframe:") {
  if (isTRUE(activity_pattern_mode) && step_size %in% c("hour", "diel", "day_night")) {
    return(sprintf(
      "<div class='timeline-window-readout'><strong>%s</strong> %s</div>",
      weather_html_escape(window_label),
      weather_html_escape(activity_pattern_window_label(step_size, view_mode, activity_pattern_index))
    ))
  }
  ...
  sprintf(
    "<div class='timeline-window-readout'><strong>%s</strong> %s to %s</div>",
    weather_html_escape(window_label),
...
spatial_map_module_ui <- function(id,
                              view = "map",
                              choices,
                              selected = NULL,
                              multiple = TRUE,
                              ...
                              surface_in_discrete = FALSE, # timeline surface: FALSE = cumulative-only
                                                           # (calendar timeline); TRUE = also in discrete
                                                           # steps (cyclic / Temporal Analysis).
                              ...
                              lock_observation_markers = FALSE,
  ...
  if (view == "select_species") {
    ...
    prediction_condition <- if (isTRUE(prediction_cumulative_only)) {
      "input.timeline_view_mode === 'cumulative'"
    } else {
      "true"
    }
    prediction_basis_condition <- paste(prediction_condition, "input.show_predicted_rai_surface", sep = " && ")

    # Reusable control sets so the grouped (boxed, renamed) and legacy ungrouped
    # renderings share one source of truth.
    monitoring_controls <- function() {
      tagList(
        ...
        if (isTRUE(include_prediction_option)) {
          conditionalPanel(
            condition = prediction_basis_condition, ns = ns,
            tags$div(
              class = "prediction-surface-options",
              radioButtons(
                inputId = ns("predicted_rai_surface_basis"),
                label = surface_basis_label(ns),
                choices = c("Location-weighted RAI" = "weighted_line_rai", "Line RAI" = "line_rai"),
    ...
    # Grouped, boxed, renamed single-group renderings (new Spatial Analysis UI).
    if (identical(layer_group, "monitoring")) {
      return(tags$div(
        class = "sa-layer-group map-display-layer-options",
        tags$div(class = "sa-layer-group-title", "Monitoring Data"),
        monitoring_controls()
      ))
    }
    if (identical(layer_group, "trapping")) {
      if (!isTRUE(trap_data_available)) return(NULL)
      return(tags$div(
        class = "sa-layer-group map-display-layer-options",
    ...
    # layer_group == "all": legacy ungrouped rendering (used by the old maps) —
    # kept verbatim so those maps are visually unchanged.
    return(
      tagList(
        if (isTRUE(include_duplicates_option)) {
          checkboxInput(
        ...
        if (isTRUE(trap_data_available) && isTRUE(include_trap_distance_option)) {
          sliderInput(
    ...
    surface_basis_cond <- paste(surface_cond, "input.show_predicted_rai_surface", sep = " && ")
    return(
      tagList(
        hr(),
        tags$div(
          ...
          if (isTRUE(include_marker_options)) {
            tagList(
              tags$strong("Monitoring Records:"),
              if (isTRUE(lock_observation_markers)) {
                shinyjs::disabled(
                  checkboxInput(
                    inputId = ns("show_observation_records"),
                    label = "Observation Records",
          ...
          if (isTRUE(include_observation_layer_options)) {
            tagList(
              ...
              conditionalPanel(
                condition = "input.timeline_view_mode === 'cumulative'",
                ns = ns,
                checkboxInput(
                  inputId = ns("show_capture_density_surface"),
                  label = "Capture Density surface",
    ...
    map_panel <- div(
      ...
      div(
        ...
        spatial_map_module_ui(primary_map_id, view = "map", map_height = "100%"),
      ...
      div(
        ...
        spatial_map_module_ui(comparative_map_id, view = "map", map_height = "100%"),
        comparison_map_label(
    ...
    # swipe via the "map" view). The bar id matches the map_only bar so the engine
    # show/hide observer works for both.
    return(
      div(
        id = ns("timeline_bar"),
        class = "timeline-bar",
...
spatial_map_module_server <- function(id,
                                  ...
                                  trap_check_frequency_override = NULL, # Reactive: for comparative density map
                                  ...
                                  timeline_mode = c("none", "always", "reactive"),
                                  ...
                                  prefer_canvas = FALSE, # Render markers on a Canvas renderer
                                                         # (much faster to draw many markers, e.g.
                                                         # thousands of trap-check circles).
                                  activity_pattern_mode = FALSE,
                                  active = NULL, # Reactive logical: when FALSE, skip the map-draw
                                                # observer so a hidden panel's analysis_dataset()
                                                # stays lazy (no recompute while not shown).
                                  overlay_side = NULL # "primary" | "comparative": which corner the
                                                      # summary/legend overlays sit in for a swipe
                                                      # panel ("primary"=left, "comparative"=right).
                                                      # NULL = infer from the map id (legacy maps).
) {
  timeline_mode <- match.arg(timeline_mode, choices = c("none", "always", "reactive"))
  
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    logger::log_debug(sprintf("spatial_map_module_server, %s moduleServer() running", id))
    ...
    last_map_update_key <- reactiveVal(NULL)
    marker_scale <- reactiveVal(0)
    ...
    observation_records_selected <- reactive({
      if (!is.null(location_markers_override) &&
          !is.null(location_markers_override())) {
        return(isTRUE(location_markers_override()))
      }

      if (is.null(input$show_observation_records)) {
        return(TRUE)
      }

      isTRUE(input$show_observation_records)
    })

    capture_records_selected <- reactive({
      ...
      if (!is.null(input$show_capture_records)) {
        return(isTRUE(input$show_capture_records))
      }
      TRUE
    })

    trap_check_frequency_requested <- reactive({
      if (!is.null(trap_check_frequency_override) &&
          !is.null(trap_check_frequency_override())) {
        return(isTRUE(trap_check_frequency_override()))
      }

      isTRUE(input$show_trap_check_frequency)
    })
    ...
    density_data_source_selected <- reactive({
      ...
      include_monitoring_layer <- if (is.null(input$show_observation_records) && is.null(input$show_predicted_rai_surface)) {
        TRUE
      } else {
      ...
      include_trapping <- isTRUE(include_trap_data_selected()) && (
        isTRUE(capture_records_selected()) ||
          isTRUE(trap_check_frequency_requested()) ||
    ...
    show_predicted_rai_surface_selected <- reactive({
      ...
      # Cumulative-only surface for ordinary timelines; activity-pattern (cyclic)
      # maps recompute the surface per discrete bin, so allow it in discrete too.
      if (timeline_active() && !identical(input$timeline_view_mode, "cumulative") &&
          !activity_pattern_mode) {
        return(FALSE)
      }
    ...
    current_selected_species <- reactive({
      if (!is.null(species_override) && !is.null(species_override())) {
        logger::log_debug(sprintf("spatial_map_module_server, %s species_override provided.", id))
        species_override()
      } else {
        logger::log_debug(sprintf("spatial_map_module_server, %s using input$selected_species.", id))
    ...
    current_selected_localities <- reactive({
      if (!is.null(localities_override) && !is.null(localities_override())) {
        logger::log_debug(sprintf("spatial_map_module_server, %s localities_override provided.", id))
        localities_override()
      } else {
        logger::log_debug(sprintf("spatial_map_module_server, %s using input$selected_localities.", id))
    ...
    calculate_bounds_from_locations <- function(active_locs_df, context_id_log, context_label) {
      if (nrow(active_locs_df) > 0 &&
        ...
        if(all(!is.infinite(c(min_lng, max_lng, min_lat, max_lat)))){
          ...
          logger::log_warn(sprintf("spatial_map_module_server [%s], ID: %s - Invalid (Infinite) bounds calculated.", context_label, context_id_log))
          return(NULL)
        }
      } else {
        logger::log_warn(sprintf("spatial_map_module_server [%s], ID: %s - Insufficient data for bounds calculation (active_locs_df empty or missing lat/lon).", context_label, context_id_log))
        return(NULL)
      }
    }
    ...
    invalidate_map_size <- function() {
      logger::log_debug(sprintf("spatial_map_module_server, ID: %s calling invalidateSize for map: %s", id, MAP_ID))
    ...
    apply_map_fit_bounds <- function() {
      ...
      bounds_desc <- if (is.null(bounds_val) || is.null(bounds_val$min_lng) || is.na(bounds_val$min_lng)) {
        "none"
      } else {
        sprintf("lng [%.5f, %.5f], lat [%.5f, %.5f]",
                bounds_val$min_lng, bounds_val$max_lng,
                bounds_val$min_lat, bounds_val$max_lat)
      }
      logger::log_debug(sprintf("spatial_map_module_server, ID: %s apply_map_fit_bounds. Bounds: %s", id, bounds_desc))
      if (!is.null(bounds_val) && !is.null(bounds_val$min_lng) && !is.na(bounds_val$min_lng)) { # Added NA check for robustness
        ...
        logger::log_info(sprintf("spatial_map_module_server, ID: %s fitBounds applied.", id))
      } else {
        logger::log_warn(sprintf("spatial_map_module_server, ID: %s no valid bounds for fitBounds.", id))
      }
    }
    ...
    recenter_map_generic <- function() {
      logger::log_info(sprintf("spatial_map_module_server, ID: %s recenter_map_generic called.", id))
      invalidate_map_size()
    ...
    output$map_display <- renderLeaflet({
      logger::log_debug(sprintf("spatial_map_module_server, %s renderLeaflet for map_display: %s", id, MAP_ID))
      leaflet::leaflet(options = leaflet::leafletOptions(preferCanvas = isTRUE(prefer_canvas))) %>%
        leaflet::addTiles(
          group = "Street",
...

    {
      ...
      timeline_active <- reactive({
        if (identical(timeline_mode, "reactive")) {
          !is.null(timeline_active_override) && isTRUE(timeline_active_override())
        } else {
          identical(timeline_mode, "always")
        }
      })

      # Show the timeline controls bar only while the timeline is live (the
      # runtime-toggle case renders the bar up-front but hidden when off).
      observe({
        shinyjs::toggle(
          selector = paste0("#", ns("timeline_bar")),
          condition = isTRUE(timeline_active())
        )
      })
      ...
      timeline_points <- reactive({
        ...
        step_size <- if (is.null(input$timeline_step_size)) {
          if (activity_pattern_mode) "hour" else "day"
        } else {
          input$timeline_step_size
        }
        ...
        timeline_time_points(
          ...
          activity_pattern_mode = activity_pattern_mode
        )
      })
      ...
      observeEvent(input$show_predicted_rai_surface, {
        req(timeline_active())
        if (!activity_pattern_mode && isTRUE(input$show_predicted_rai_surface)) {
      ...
      observeEvent(input$show_observation_records, {
        req(timeline_active())
        if (!activity_pattern_mode && !isTRUE(input$show_observation_records)) {
          current_step <- if (is.null(input$timeline_step_size)) "day" else input$timeline_step_size
          if (current_step %in% c("hour", "day_night", "diel", "day")) {
            updateSelectInput(session, "timeline_step_size", selected = "week")
          }
      ...
      observeEvent(input$playback_tick, {
        ...
        if (activity_pattern_mode) {
          step_size <- if (is.null(input$timeline_step_size)) "hour" else input$timeline_step_size
          if (step_size %in% c("hour", "diel", "day_night")) {
            bins <- activity_pattern_bins(step_size)
            current_index <- timeline_selected_index(input$time_slider, length(bins))
            if (current_index < length(bins)) {
              updateSliderInput(session, "time_slider", value = current_index + 1)
            } else {
              is_playing(FALSE)
            }
            return()
      ...
      analysis_dataset <- reactive({
        ...
        logger::log_debug(sprintf(
          "spatial_map_module_server [density], %s analysis_dataset() for species: %s, localities: %s",
          id, paste(species_dens, collapse=", "), paste(localities_dens, collapse=", ")
        ))
        .ad_t0 <- Sys.time()  # TIMING: analysis_dataset data-prep start
        ...
        timeline_window_text_dens <- NULL
        ...
        if (use_timeline) {
          ...
          if (activity_pattern_mode && step_size %in% c("hour", "diel", "day_night")) {
            bins <- activity_pattern_bins(step_size)
            current_index <- timeline_selected_index(input$time_slider, length(bins))
            bin_column <- activity_pattern_bin_column(step_size)

            add_bin_column <- function(df) {
              if (identical(bin_column, "activity_pattern_hour_bin")) {
                df %>% dplyr::mutate(activity_pattern_hour_bin = format(timestamp, "%H", tz = timeline_actual_timezone()))
              } else {
                df
              }
            }

            obs_filtered_dens <- add_bin_column(obs_filtered_dens)
            obs_cumulative_dens <- add_bin_column(obs_cumulative_dens)
            obs_for_scale_dens <- add_bin_column(obs_for_scale_dens)

            cumulative_bins <- bins[seq_len(current_index)]
            active_bins <- if (identical(view_mode_dens, "single")) bins[current_index] else cumulative_bins

            obs_cumulative_dens <- obs_cumulative_dens %>% dplyr::filter(.data[[bin_column]] %in% cumulative_bins)
            obs_filtered_dens   <- obs_filtered_dens   %>% dplyr::filter(.data[[bin_column]] %in% active_bins)
            obs_for_scale_dens  <- obs_for_scale_dens  %>% dplyr::filter(.data[[bin_column]] %in% active_bins)

            timeline_window_text_dens <- activity_pattern_window_label(step_size, view_mode_dens, current_index)
            current_time_dens <- timeline_period_end()
          } else {
            ...
            start_time_dens <- if (identical(view_mode_dens, "single")) {
              if (step_size %in% c("day_night", "diel")) {
        ...
        if (isTRUE(show_trapping_density)) {
          ...
          if (nrow(trap_observations_dens) > 0 && "trap_marker_type" %in% names(trap_observations_dens)) {
            ...
            metrics_trap_marker_types_dens <- union(
              ...
              c(
                # CDS anchors on every checked location (captures + blank checks),
                # so the surface sees the zeros, not just the capture hotspots.
                if (isTRUE(show_capture_density_surface_selected())) "capture",
                if (isTRUE(show_capture_density_surface_selected())) "check"
              )
            )
            trap_observations_for_metrics_dens <- trap_observations_dens %>%
              dplyr::filter(.data$trap_marker_type %in% metrics_trap_marker_types_dens)
            trap_observations_dens <- trap_observations_dens %>%
          ...
          if (isTRUE(show_trap_check_frequency_selected())) {
              ...
              check_summary_dens <- check_summary_dens %>%
                dplyr::mutate(
                  trap_checks_per_100_trap_days = dplyr::if_else(
                    is.finite(.data$trap_days) & .data$trap_days > 0,
          ...
          if (isTRUE(show_trap_check_frequency_selected()) && nrow(check_summary_dens) > 0) {
            trap_check_heatmap_data_dens <- check_summary_dens %>%
              ...
              dplyr::select(
                longitude, latitude, locationName, locality, line, trap_line, trap_type_desc,
                locality_match_type, locality_distance_km, nearest_monitoring_locationName,
                trap_checks, trap_days, first_check, last_check, check_span_days,
                mean_check_interval_days,
                selected_species_capture_count, any_species_capture_count,
          ...
          # Record the selected-period window on every trap-support row so the
          # popup can show it generically when a trap has no overlapping checks.
          if (nrow(trap_support_locations_dens) > 0) {
            trap_support_locations_dens$window_start_date <- as.character(trap_start_date)
            trap_support_locations_dens$window_end_date <- as.character(trap_end_date)
          }
          trap_capture_summary_dens <- if (isTRUE(capture_records_selected()) ||
          ...
          if (nrow(trap_capture_summary_dens) > 0) {
            ...
            trap_density_summary_dens <- trap_capture_summary_dens %>%
              ...
              dplyr::summarise(
                ...
                line = as.character(dplyr::first(.data$line)),
                trap_type_desc = dplyr::first(.data$trap_type_desc),
        ...
        dens_time_key <- if (use_timeline && !is.null(current_time_dens)) {
          format(current_time_dens, "%Y-%m-%d %H:%M:%S", tz = timeline_actual_timezone())
        } else {
          "current-selection"
        }
        ...
        if (isTRUE(show_predicted_rai_surface_selected())) {
          ...
          # Activity-pattern surfaces share one current_time (the period end) across
          # every repeating bin, so the bin must key the cache or Day/Night would
          # return each other's stale surface.
          surface_bin_key <- if (use_timeline && activity_pattern_mode &&
                                 step_size %in% c("hour", "diel", "day_night")) {
            paste(sort(active_bins), collapse = ",")
          } else {
            "all"
          }
          ...
          if (!is.null(surface_cache) && identical(surface_cache$key, surface_cache_key)) {
            ...
            if (use_timeline) {
              ...
              # Cyclic / Temporal Analysis: restrict the surface to the active
              # repeating bin(s) so it RECALCULATES per discrete step (e.g. Day vs
              # Night), not just over the cumulative calendar window. (add_bin_column,
              # bin_column and active_bins come from the timeline block above and only
              # exist in activity-pattern mode.)
              if (activity_pattern_mode && step_size %in% c("hour", "diel", "day_night")) {
                surface_obs_filtered_dens <- add_bin_column(surface_obs_filtered_dens) %>%
                  dplyr::filter(.data[[bin_column]] %in% active_bins)
              }
        ...
        capture_density_surface_cache_key <- "surface-off"
        if (isTRUE(show_capture_density_surface_selected())) {
          capture_cache_key <- paste(
            id,
            if (use_timeline) "timeline-step" else "static",
            paste(sort(species_dens), collapse = ","),
            paste(sort(localities_dens), collapse = ","),
          ...
          capture_density_surface_cache_key <- capture_cache_key

          capture_cache <- capture_density_surface_cache()
          if (!is.null(capture_cache) && identical(capture_cache$key, capture_cache_key)) {
            capture_density_surface <- capture_cache$surface
            capture_density_surface_message <- capture_cache$message
          } else {
            # Anchor the IDW on EVERY checked location within range (capture +
            ...
            if (nrow(usable_capture_locations) < 3) {
              ...
              capture_density_surface <- create_idw_prediction_surface(
                ...
                grid_n = surface_grid_n,
                buffer_m = 75,
                output_col = "predicted_capture_density"
              )
            ...
            capture_density_surface_cache(
              list(
                key = capture_cache_key,
                surface = capture_density_surface,
                message = capture_density_surface_message
              )
            )
          }
        }
        ...
        marker_scale(shared_absolute_max)

        # One count table per visible record type, each gated by its own toggle
        # so the overlay always matches what the map draws. Summing the two metrics
        # into one table would be meaningless, so when both show they stack (a gap
        # between them comes from CSS adjacent-sibling margin).
        ...
        show_monitoring_marker_layer <- show_monitoring_observations
        ...
        logger::log_info("TIMING [%s] analysis_dataset data prep (filter+trap fetch+summaries+surfaces): %.2fs",
                         id, as.numeric(difftime(Sys.time(), .ad_t0, units = "secs")))
        ...
        list(
          ...
          map_update_key = paste(
            ...
            if (is.null(timeline_window_text_dens)) "no-window" else timeline_window_text_dens,
            exclude_possible_duplicates_selected(),
            ...
            shared_absolute_max,
            trap_locality_distance_km_selected(),
            capture_records_selected(),
            show_trap_unchecked_locations_selected(),
            observation_records_selected(),
      ...
      observe({
        # Skip while this panel is hidden so analysis_dataset() is never read
        # (and therefore not computed) until the panel is shown. Reading active()
        # here (and NOT the dataset) means dataset invalidations while hidden do
        # not re-trigger this observer; on show it recomputes only if stale.
        if (!is.null(active) && !isTRUE(active())) {
          return()
        }
        req(analysis_dataset())
        logger::log_debug(sprintf("spatial_map_module_server [density], %s observer updating map content on %s", id, MAP_ID))
        data_for_map <- analysis_dataset()

        if (!is.null(data_for_map$map_update_key) &&
            identical(last_map_update_key(), data_for_map$map_update_key)) {
        ...
        last_map_update_key(data_for_map$map_update_key)
        ...
        update_map(
          ...
          species_display_mode = data_for_map$species_display_mode,
          all_marker_data_with_warnings = data_for_map$separate_markers,
      ...
      output$timeline_window_ui <- renderUI({
        ...
        if (activity_pattern_mode && step_size %in% c("hour", "diel", "day_night")) {
          current_index <- timeline_selected_index(input$time_slider, length(activity_pattern_bins(step_size)))
          return(HTML(timeline_window_readout(NULL, NULL, step_size, NULL, view_mode,
            activity_pattern_mode = TRUE, activity_pattern_index = current_index)))
        }
      ...
      # Keep the timeline slider/window rendered even when their bar is hidden, so a
      # shared-clock comparison can sync a hidden slave panel's slider.
      outputOptions(output, "timeline_slider_ui", suspendWhenHidden = FALSE)
      outputOptions(output, "timeline_window_ui", suspendWhenHidden = FALSE)

      # Current-step season + monitoring-window readout. Exposed as a reactive (HTML
      # string, or NULL) so the workbench can render it INSIDE the on-map identity
      # heading overlay (one box that grows when timeline is on), rather than as a
      # separate map control. Lightweight, derived from the slider like the window.
      timeline_status_html <- reactive({
        if (!isTRUE(timeline_active()) || is.null(input$time_slider)) return(NULL)
        step_size <- if (is.null(input$timeline_step_size)) {
          if (activity_pattern_mode) "hour" else "day"
        } else {
          input$timeline_step_size
        ...
        # When a single season is selected the season label/dates just repeat the
        # period, so show only the Monitoring sub-window (a genuine sub-period).
        selected_period_count <- if (is.function(period_names)) {
          length(stats::na.omit(period_names()))
        } else {
          NA_integer_
        }
        render_timeline_period_control(
          timeline_period_status(current_time, timeline_period_groups()),
          monitoring_window,
          monitoring_only = !is.na(selected_period_count) && selected_period_count == 1
        )
      })
      ...
      root_session$userData$pdf_export_density_map_renderers[[MAP_ID]] <- function(width = NULL, height = NULL, export_dir = NULL) {
        data_for_map <- isolate(analysis_dataset())
      ...
      return(list(
        ...
        # Insights from the same memoised reactive the map renders from.
...
update_map <- function(map_id = NULL,
                               ...
                               source_map_id = NULL,
                               overlay_side = NULL,
                               species_display_mode = "combined",
                               all_marker_data_with_warnings = NULL,
  ...
  # Which side of a side-by-side swipe this map occupies, deciding where its
  ...
  if (!is.null(weather_control_html)) {
    # Bottom corner, on this map's own side (left map -> bottom-left, right map ->
  ...
  # Legacy maps show the current-step season / monitoring readout as a map overlay.
  # The new workbench passes period_control_html = NULL and shows it in the timeline
  # bar instead (output$timeline_period_status_ui), to declutter the map.
  ...
  if (nrow(obs_summary_location) > 0) {
    ...
    logger::log_debug(sprintf("spatial_map_module_server, %s update_map() has no results available", map_id))
    max_count <- if (!is.null(absolute_max)) absolute_max else NA
    
    # Show "No results!" message
    has_trap_check_layer <- isTRUE(show_trap_check_frequency) &&
      !is.null(trap_check_heatmap_data) && nrow(trap_check_heatmap_data) > 0
    if ((!show_zero || nrow(active_locations) == 0) &&
        !isTRUE(has_trap_support_data) &&
        !has_trap_check_layer &&
        !identical(species_display_mode, "separate")) {
      logger::log_debug(sprintf("spatial_map_module_server, %s update_map() creating popup saying no results", map_id))
      proxy %>% addControl(
  ...
  obs_summary_location <- obs_summary_location %>%
    mutate(
      ...
      density_border_opacity = ifelse(marker_value > 0, 0.9, 0.25)
    )

  density_marker_color <- "#dc2626"
  pal_domain_max <- if (!is.null(absolute_max) && is.finite(absolute_max) && absolute_max > 0) {
    absolute_max
  } else if (!is.na(max_marker_value) && max_marker_value > 0) {
    max_marker_value
  } else {
    1
  }
  pal_non_trapped <- colorNumeric(c("#93c5fd", "#1d4ed8"), domain = c(0, pal_domain_max))
  ...
  obs_summary_location <- obs_summary_location %>%
    dplyr::mutate(
      marker_color = dplyr::case_when(
        .data$marker_dataset == "trap_capture"           ~ pal_trap(pmax(.data$marker_value, 0)),
        .data$marker_dataset == "monitoring_trapped"     ~ pal_trapped(pmax(.data$marker_value, 0)),
        .data$marker_dataset == "monitoring_non_trapped" ~ pal_non_trapped(pmax(.data$marker_value, 0)),
        TRUE ~ density_marker_color
      ),
      marker_border_color = dplyr::case_when(
  ...
  if (!is.null(capture_density_surface) && nrow(capture_density_surface) > 0) {
    ...
    capture_surface_pal <- colorNumeric(
      palette = "RdPu",
  ...
  intensity_scale_max <- NA_real_

  heatmap_gradient <- function(hex_color) {
    c("0" = grDevices::adjustcolor(hex_color, alpha.f = 0), "1" = hex_color)
  }

  render_intensity_circles <- function(proxy, data, base_color, group_name, scale_max = NULL) {
    data <- data[is.finite(data$value) & data$value > 0, , drop = FALSE]
    if (nrow(data) == 0) return(proxy)
    local_max <- max(data$value, na.rm = TRUE)
    max_val <- if (!is.null(scale_max) && is.finite(scale_max) && scale_max > local_max) scale_max else local_max
    if (!is.finite(max_val) || max_val <= 0) max_val <- 1
    intensity_scale_max <<- if (is.na(intensity_scale_max)) max_val else max(intensity_scale_max, max_val)
    pal <- leaflet::colorNumeric(palette = c("#f0f0f0", base_color), domain = c(0, max_val))
    proxy %>% leaflet::addCircles(
      data = data, lng = ~longitude, lat = ~latitude,
      radius = 75,
      fillColor = ~pal(value), fillOpacity = 0.85,
      color = base_color, weight = 1.5, opacity = 0.7,
      popup = if ("popup_html" %in% names(data)) ~popup_html else NULL,
      label = if ("label_text" %in% names(data)) ~label_text else NULL,
      group = group_name
    )
  }
  ...
  marker_popup_html <- function(marker_data, include_review_link = TRUE) {
    ...
    vapply(seq_len(nrow(marker_data)), function(i) {
      ...
      count_val <- suppressWarnings(as.numeric(marker_field("count", i)))
      camera_hours_val <- suppressWarnings(as.numeric(marker_field("camera_hours", i)))
      line_camera_hours_val <- suppressWarnings(as.numeric(marker_field("line_camera_hours", i)))
      line_rai_val <- suppressWarnings(as.numeric(marker_field("line_rai", i)))
      location_line_share <- suppressWarnings(as.numeric(marker_field("location_line_count_share", i)))

      # Header: OH 1_4 @ Line 1 · Ohope
      header_parts <- character(0)
      if (nzchar(line_text)) header_parts <- c(header_parts, line_text)
      if (nzchar(locality_text)) header_parts <- c(header_parts, locality_text)
      header_suffix <- if (length(header_parts) > 0) paste0(" @ ", paste(header_parts, collapse = " · ")) else ""

      # "2 selected observations · 100% of line"
      share_pct <- if (!is.na(location_line_share) && is.finite(location_line_share)) {
        paste0(" · ", round(location_line_share * 100), "% of line")
      } else ""
      obs_html <- paste0(
        "<br><strong>", format_marker_value(count_val, digits = 0),
        "</strong> selected observations", share_pct
      )

      # "Line RAI: 1.99 per 2,000 camera hours"
      rai_html <- if (!is.na(line_rai_val) && is.finite(line_rai_val)) {
        rai_fmt <- format(round(line_rai_val, 2), nsmall = 2, trim = TRUE)
        norm_fmt <- if (!is.na(rai_norm_hours) && is.finite(rai_norm_hours)) {
          format(as.integer(rai_norm_hours), big.mark = ",", trim = TRUE)
        } else "100"
        paste0("<br>Line RAI: <strong>", htmltools::htmlEscape(rai_fmt),
               "</strong> per ", norm_fmt, " camera hours")
      } else ""

      # "499.5 h location · 2,007.9 h line"
      effort_html <- paste0(
        format_marker_value(camera_hours_val, digits = 1), " h location",
        if (!is.na(line_camera_hours_val) && is.finite(line_camera_hours_val)) {
          paste0(" · ", format_marker_value(line_camera_hours_val, digits = 1), " h line")
        } else ""
      )

      paste0(
        "<strong>", location_text, "</strong>", header_suffix,
        obs_html,
        rai_html,
        "<br>", effort_html,
        review_link
  ...
  marker_label_text <- function(marker_data) {
    vapply(seq_len(nrow(marker_data)), function(i) {
      loc_name <- as.character(marker_data$locationName[[i]])
      if (is.na(loc_name)) loc_name <- ""
      ...
      if (identical(marker_dataset, "trap_capture")) {
        sel_cap <- suppressWarnings(as.numeric(
          if ("selected_species_capture_count" %in% names(marker_data)) marker_data$selected_species_capture_count[[i]]
          else if ("count" %in% names(marker_data)) marker_data$count[[i]]
          else NA_real_
        ))
        trap_chk <- suppressWarnings(as.numeric(
          if ("trap_checks" %in% names(marker_data)) marker_data$trap_checks[[i]] else NA_real_
        ))
        parts <- character(0)
        if (!is.na(sel_cap) && is.finite(sel_cap) && sel_cap > 0) {
          parts <- c(parts, paste0("Species captures: ", as.integer(sel_cap)))
        }
        if (!is.na(trap_chk) && is.finite(trap_chk)) {
          parts <- c(parts, paste0("Trap checks: ", as.integer(trap_chk)))
        }
        if (length(parts) == 0) return(loc_name)
        paste0(loc_name, " — ", paste(parts, collapse = ", "))
      } else {
        obs_count <- suppressWarnings(as.numeric(
          if ("count" %in% names(marker_data)) marker_data$count[[i]] else NA_real_
        ))
        n_deps <- suppressWarnings(as.numeric(
          if ("n_deployments" %in% names(marker_data)) marker_data$n_deployments[[i]] else NA_real_
        ))
        parts <- character(0)
        if (!is.na(obs_count) && is.finite(obs_count) && obs_count > 0) {
          parts <- c(parts, paste0("Observations: ", as.integer(obs_count)))
        }
        if (!is.na(n_deps) && is.finite(n_deps) && n_deps > 0) {
          parts <- c(parts, paste0("Deployments: ", as.integer(n_deps)))
        }
        if (length(parts) == 0) return(loc_name)
        paste0(loc_name, " — ", paste(parts, collapse = ", "))
      }
  ...
  # Render effort intensity first so capture and observation intensity render on top
  if (isTRUE(show_trap_check_frequency) && !is.null(trap_check_heatmap_data) && nrow(trap_check_heatmap_data) > 0) {
    check_data <- trap_check_heatmap_data %>% dplyr::mutate(value = .data$trap_checks)
    chk_vec <- suppressWarnings(as.numeric(check_data$trap_checks))
    cap_vec <- suppressWarnings(as.numeric(check_data$selected_species_capture_count))
    check_data$label_text <- paste0(
      as.character(check_data$locationName), " — ",
      ifelse(!is.na(cap_vec) & is.finite(cap_vec) & cap_vec > 0,
             paste0("Species captures: ", as.integer(cap_vec), ", "), ""),
      "Trap checks: ", ifelse(!is.na(chk_vec) & is.finite(chk_vec), as.integer(chk_vec), "0")
    )
  ...
  circle_locations <- if (nrow(obs_summary_location) > 0) obs_summary_location[0, , drop = FALSE] else data.frame(marker_dataset = character(0))

  if (identical(species_display_mode, "separate") && !is.null(all_marker_data_with_warnings)) {
    # Per-species marker rendering (Separate mode)
    ...
    if (length(all_marker_data) > 0) {
      for (species_data_key in names(all_marker_data)) {
        ...
        if (!is.null(markers) && length(markers) > 0) {
          logger::log_debug(sprintf("update_map() separate mode for map_id: %s - Adding %d markers for species: %s",
                                    map_id, length(markers), species_data$species))
    ...
    # Combined mode: aggregated density circles
    if (isTRUE(show_observation_records) || isTRUE(has_trap_density_layers) || isTRUE(has_trap_support_layers)) {
      obs_summary_location$popup_content <- marker_popup_html(obs_summary_location, include_review_link = TRUE)
      obs_summary_location$label_content <- marker_label_text(obs_summary_location)
      ...
      add_density_circles <- function(locations) {
        ...
        proxy %>%
          addCircles(
            ...
            fillOpacity = ~pmax(0, pmin(0.82, (marker_value / pal_domain_max) * 0.82)),
      ...
      if (density_data_source %in% c("trapping", "both") &&
        ...
        support_popup <- vapply(seq_len(nrow(trap_support_locations)), function(i) {
          ...
          window_start <- if ("window_start_date" %in% names(support_record)) support_record$window_start_date else NULL
          window_end <- if ("window_end_date" %in% names(support_record)) support_record$window_end_date else NULL
          paste0(
            "<strong>", htmltools::htmlEscape(safe_marker_value(support_record$locationName)), "</strong><br>",
            ...
            "<br><br>",
            trap_metrics_popup_html(support_record, "selected species", window_start = window_start, window_end = window_end)
          )
        }, character(1))
        trap_support_locations$popup_content <- support_popup

        check_counter_locs <- trap_support_locations[trap_support_locations$density_support_type == "check", , drop = FALSE]
      ...
      if (isTRUE(show_observation_records) && show_zero && identical(marker_metric, "count")) {
  ...
  has_intensity_layers <- length(heatmap_visible_layers) > 0 && is.finite(intensity_scale_max) && intensity_scale_max > 0
  if (!identical(species_display_mode, "separate") && length(visible_layers) > 0 &&
      ((!is.na(max_marker_value) && max_marker_value > 0) || has_intensity_layers)) {
    legend_rows <- c()
    ...
    if ("trap_capture" %in% visible_layers) {
      legend_rows <- c(legend_rows, "<div class='trap-marker-legend-row'><span class='trap-marker-legend-swatch' style='background:#dc2626;'></span>Capture Counts</div>")
    }
    ...
    intensity_scale_text <- if (has_intensity_layers) {
      paste0(
        ...
        "<span style='white-space: nowrap;'>Full colour at:&nbsp;<strong>", htmltools::htmlEscape(format_scale_val(intensity_scale_max)), "</strong></span>",
        "</div></div>"
      )
    } else {
      ""
...
extract_trap_tag_value <- function(tags, key) {
  # Vectorised replacement for the previous per-row vapply() + strsplit().
  # Matches "<key>:<value>" at the start of the string or just after a "|"
  # delimiter, capturing up to the next "|"; returns NA when the key is absent.
  # str_match returns NA for rows where the key is absent (or the tag is NA/empty),
  # matching the original. A present-but-empty value stays "" (the original
  # returned "" too, which downstream coalesce() relies on).
  tags <- as.character(tags)
  pattern <- paste0("(?:^|\\|\\s*)", key, ":\\s*([^|]*)")
  trimws(stringr::str_match(tags, pattern)[, 2])
}

trap_marker_group <- function(description) {
  # Vectorised: previously this ran a per-row vapply() with up to 12 grepl()
  # calls each (~28k grepl() calls for a few thousand trap rows). Now each
  # pattern is one vectorised grepl() over the whole column. Priority is
  # first-match-wins (stoat highest), achieved by assigning in reverse priority
  # so higher-priority patterns overwrite. fixed = TRUE matches the original
  # plain-substring grepl() semantics and is faster.
  description <- tolower(ifelse(is.na(description), "", description))
  patterns <- c("stoat", "weasel", "ferret", "rat", "mouse", "cat",
                "rabbit", "hedgehog", "possum", "weka", "bird", "mustelid")
  result <- rep("other", length(description))
  for (i in rev(seq_along(patterns))) {
    result[grepl(patterns[[i]], description, fixed = TRUE)] <- patterns[[i]]
  }
  result
}
...
prepare_trap_observations_for_map <- function(trap_data_value,
  ...
  trap_deployment_fields <- trap_deps %>%
    dplyr::transmute(
      ...
      trap_type_desc = if ("trap_type_desc" %in% names(.)) trap_type_desc else NA_character_,
  ...
  if (isTRUE(include_blank_checks)) {
    ...
    blank_checks <- trap_obs %>%
      ...
      # First row per deployment. distinct(.keep_all) is hash-based and far faster
      # than group_by(deploymentID) %>% slice(1) over the full all-time trap dataset
      # (that pattern was ~6s for ~1000 deployments; this is near-instant).
      dplyr::distinct(.data$deploymentID, .keep_all = TRUE) %>%
      add_trap_source_fields() %>%
      dplyr::mutate(
    ...
    if (nrow(unchecked_locations) > 0) {
      unchecked_rows <- unchecked_locations %>%
        dplyr::transmute(
          ...
          trap_checks = 0L,
          first_check = as.Date(NA),
          last_check = as.Date(NA),
          check_span_days = NA_integer_,
...
create_trap_location_popup_content <- function(trap_record, capture_links_html = "") {
  ...
  record_field <- function(field, default = NA) {
    if (!field %in% names(trap_record)) return(default)
    trap_record[[field]][[1]]
  }
  ...
  first_available <- function(fields, default = NA) {
    for (field in fields) {
      ...
      if (!is.null(value) && length(value) > 0 && !is.na(value) && nzchar(as.character(value))) return(value)
    }
    default
  }
  ...
  # Line number: use numeric line column, fall back to trap_line only if line is missing
  line_number <- as.character(first_available(c("line"), ""))
  if (!nzchar(line_number) || line_number == "NA") {
    line_number <- trimws(gsub("(?i)^line\\s*", "", as.character(first_available(c("trap_line"), "")), perl = TRUE))
  }

  # Trap type (e.g. DOC200)
  trap_type <- htmltools::htmlEscape(safe_marker_value(record_field("trap_type_desc", "")))

  # Line 1: DF03 @ Line 18 · DOC200
  header_html <- paste0(
    "<strong>", location_text, "</strong>",
    if (nzchar(line_number)) paste0(" @ Line ", htmltools::htmlEscape(line_number)) else "",
    if (nzchar(trap_type)) paste0(" · ", trap_type) else ""
  )

  # Line 2: locality with match type context
  locality_match <- as.character(record_field("locality_match_type", ""))
  locality_distance <- suppressWarnings(as.numeric(record_field("locality_distance_km", NA_real_)))
  nearest_name <- htmltools::htmlEscape(safe_marker_value(record_field("nearest_monitoring_locationName", "")))
  locality_detail <- if (identical(locality_match, "within")) {
    paste0(locality_text, " (within)")
  } else if (identical(locality_match, "nearest") && nzchar(nearest_name) &&
             !is.na(locality_distance) && is.finite(locality_distance)) {
    paste0(locality_text, " (", format_trap_metric_number(locality_distance, 1), " km to ", nearest_name, ")")
  } else {
    locality_text
  }
  locality_html <- if (nzchar(locality_detail)) paste0("<br>", locality_detail) else ""

  # Captures: 1 selected (4 total)
  ...
  any_captures <- suppressWarnings(as.numeric(record_field("any_species_capture_count", NA_real_)))
  captures_html <- if (!is.na(selected_captures) && is.finite(selected_captures)) {
    total_part <- if (!is.na(any_captures) && is.finite(any_captures) && any_captures != selected_captures) {
      paste0(" (", format_trap_metric_number(any_captures), " total)")
    } else ""
    paste0("<br><strong>", format_trap_metric_number(selected_captures), "</strong> selected", total_part, " captures")
  } else ""

  # Rate: 1.3 captures / 100 trap-days, or "Insufficient data" when unavailable
  rate_val <- suppressWarnings(as.numeric(record_field("captures_per_100_trap_days_selected_species", NA_real_)))
  rate_html <- if (!is.na(rate_val) && is.finite(rate_val)) {
    paste0("<br>", format_trap_metric_rate(rate_val), " captures / 100 trap-days")
  } else {
    "<br>Insufficient data / 100 trap-days"
  }

  # Checks · days · avg
  trap_checks_val <- suppressWarnings(as.numeric(record_field("trap_checks", NA_real_)))
  check_span_val <- suppressWarnings(as.numeric(record_field("check_span_days", NA_real_)))
  if (is.na(check_span_val) || !is.finite(check_span_val)) {
    first_d <- as.Date(record_field("first_check", NA))
    last_d <- as.Date(record_field("last_check", NA))
    if (!is.na(first_d) && !is.na(last_d)) check_span_val <- as.integer(last_d - first_d)
  }
  avg_val <- suppressWarnings(as.numeric(record_field("mean_check_interval_days", NA_real_)))
  checks_html <- if (!is.na(trap_checks_val) && is.finite(trap_checks_val) && trap_checks_val > 0) {
    parts <- paste0(format_trap_metric_number(trap_checks_val), " checks")
    if (!is.na(check_span_val) && is.finite(check_span_val)) {
      parts <- paste0(parts, " · ", format_trap_metric_number(check_span_val), " days")
    }
    if (!is.na(avg_val) && is.finite(avg_val)) {
      parts <- paste0(parts, " · ", format_trap_metric_number(avg_val, 1), " d avg")
    }
    paste0("<br>", parts)
  } else ""

  # Date range: 11 Feb 2026–9 Apr 2026
  first_date <- as.Date(record_field("first_check", NA))
  last_date <- as.Date(record_field("last_check", NA))
  date_html <- if (!is.na(first_date) && !is.na(last_date)) {
    fmt_d <- function(d) format(d, "%-d %b %Y")
    paste0("<br>", fmt_d(first_date), "–", fmt_d(last_date))
  } else ""

  # Capture records
  capture_links_html <- safe_marker_value(capture_links_html)
  capture_records_html <- if (nzchar(capture_links_html)) {
...
trap_metrics_popup_html <- function(trap_record, selected_species_label = "selected species",
  ...
  if (is.na(check_count_value) || check_count_value == 0) {
    ...
    # Generic period-window fallback: with no overlapping checks there is no
    # check coverage to report, so show the selected-period window instead.
    window_first <- suppressWarnings(as.Date(window_start))
    window_last <- suppressWarnings(as.Date(window_end))
    window_range <- if (length(window_first) == 1 && length(window_last) == 1 &&
                        !is.na(window_first) && !is.na(window_last)) {
      htmltools::htmlEscape(format_trap_date_range(window_first, window_last, NA))
    } else {
      date_range
    }
    ...
    return(paste0(
      ...
      "Window: <strong>", window_range, "</strong><br>",
...
summarise_visible_trap_metrics <- function(trap_observations) {
  ...
  if (!"trap_type_desc" %in% names(trap_rows)) trap_rows$trap_type_desc <- NA_character_
  if (!"line" %in% names(trap_rows)) trap_rows$line <- NA_integer_
  ...
  trap_rows %>%
    ...
    dplyr::summarise(
      ...
      locality_distance_km = dplyr::first(.data$locality_distance_km),
      nearest_monitoring_locationName = dplyr::first(.data$nearest_monitoring_locationName),
      dplyr::across(dplyr::all_of(trap_metric_columns()), dplyr::first),
      .groups = "drop"
    ) %>%
    dplyr::mutate(
...
create_trap_capture_summary <- function(trap_observations) {
  ...
  if (!"trap_type_desc" %in% names(capture_rows)) capture_rows$trap_type_desc <- NA_character_
  if (!"line" %in% names(capture_rows)) capture_rows$line <- NA_integer_
