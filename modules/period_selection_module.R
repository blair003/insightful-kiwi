...
period_selection_module_server <- function(id, 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$period_selection, {
      logger::log_info(sprintf("period_selection_module_server() period_selection changing for %s, new period is %s", id, input$period_selection))
    ...
    for (summary_output_id in summary_output_ids) {
      output[[summary_output_id]] <- renderUI({
        ...
        formatted_start_date <- paste0("<strong>", format(start_date_value, "%d %b %Y"), " (", format(start_date_value, "%a"), ")</strong>")
        formatted_end_date <- paste0("<strong>", format(end_date_value, "%d %b %Y"), " (", format(end_date_value, "%a"), ")</strong>")
        ...
        kable_table <- knitr::kable(dates_table, format = "html", col.names = NULL, escape = FALSE)
