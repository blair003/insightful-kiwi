...
generate_html_header <- function() {
  logo_path <- report_image_path(params$config$meta$organisation_header_logo)
...
report_image_path <- function(...) {
  normalizePath(file.path(params$config$paths$report_template_resources, "images", ...), mustWork = FALSE)
}
...
Count and RAI basis: <strong>`r params$data$count_basis_label`</strong>.
...
generate_column_descriptions(c("Species Class", "RAI ± SE", "Net Data Basis"))
...
#    cat('### Density heatmap\n')
  # Use HTML <img> tag to embed the image
  cat(sprintf('<img src="%s" alt="%s">', params$data$abundance_maps[[species_name]], species_name), '<br>\n')
...
<img src="`r report_image_path('kohipoint_map.jpg')`" alt="Map of Kohi Point Scenic Reserve showing camera monitoring lines" width="1024" border="1" />
...
<img src="`r report_image_path('mokoroa_map.jpg')`" alt="Map of Mokoroa Bush Scenic Reserve showing camera monitoring lines" width="1024" border="1" />
<figcaption>Source: Gaye Payze, Whakatane Kiwi Trust</figcaption>
</figure>
...
<img src="`r report_image_path('ohope_map.jpg')`" alt="Map of Ohope Scenic Reserve showing camera monitoring lines" width="1024" border="1" />
<figcaption>Source: Gaye Payze, Whakatane Kiwi Trust</figcaption>
</figure>
...
<figure>
<img src="`r report_image_path('example_camera_setup_in_field.jpg')`" alt="Image showing an example camera deployed, camera strapped to tree and bait basket in view" width="800" border="1" />
